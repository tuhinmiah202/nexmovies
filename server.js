...
const app = express();
const PORT = process.env.PORT || 7860; // Match fly.toml
...
// --- TRANSPARENT VIDEO PROXY (Bypass Blocks & CORS) ---
...
// --- API ROUTES (STRICTLY FROM YOUR BACKEND) ---
app.get('/api/home', async (req, res) => {
  try {
    const r = await fetch(`${API_URL}/home`).then(res => res.json());
    if (!r.sections) return res.json({ sections: [] });
...
app.get('/api/stream', async (req, res) => {
  ...
  try {
    ...
    if (r && r.has_resource) {
      ...
      if (r.sources) r.sources.forEach(s => { if (s.url) s.url = `${host}/api/proxy?url=${encodeURIComponent(s.url)}`; });
